...
impl<'ast> TypeCompiler<'ast> {
    ...
    fn visit_class(&mut self, class: &ClassDecl<'ast>) {
        ...
        for member in class.members {
            if let ClassMember::Method(method) = member {
                ...
                for &id in all_funcs {
                    if self.context.get_function(id).object_type == Some(type_id) {
                        func_ids.push(id);
                    }
                }
        ...
        for &ctor_id in &constructors {
            let func = self.context.get_function(ctor_id);
        ...
        if let Some(op_assign_id) = self.context.find_operator_method(type_id, OperatorBehavior::OpAssign) {
            let func = self.context.get_function(op_assign_id);
    ...
    fn resolve_type_expr(&mut self, expr: &TypeExpr<'ast>) -> Option<DataType> {
        ...
        let type_id = if !expr.template_args.is_empty() {
            ...
            // TODO(Phase 6.4): Template instantiation moves to CompilationContext
            // For now, template instantiation is not supported - it requires access to both
            // FfiRegistry (for template definitions like array<T>) and ScriptRegistry
            self.errors.push(SemanticError::new(
                SemanticErrorKind::NotATemplate,
                expr.span,
                "Template instantiation requires CompilationContext (Phase 6.4)".to_string(),
            ));
            return None;
        } else {
            base_type_id
        ...
        for member in class.members {
            if let ClassMember::Method(method) = member {
                ...
                if let Some(final_method_id) = self.context.is_base_method_final(type_id, method_name) {
                    let base_func = self.context.get_function(final_method_id);
                    self.errors.push(SemanticError::new(
...
mod tests {
    ...
    fn mixin_basic_method() {
        ...
        let method_names: Vec<&str> = methods.iter()
            .map(|&id| data.context.get_function(id).name.as_str())
            .collect();
    ...
    fn mixin_class_method_overrides_mixin() {
        ...
        let update_count = methods.iter()
            .filter(|&&id| data.context.get_function(id).name == "update")
            .count();
