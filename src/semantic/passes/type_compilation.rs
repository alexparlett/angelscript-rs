...
impl<'ast> TypeCompiler<'ast> {
        ...
        for member in class.members {
            if let ClassMember::Field(field) = member
                && let Some(field_type) = self.resolve_type_expr(&field.ty) {
                    ...
                    if field_type.type_hash == primitive_hashes::VOID {
                        self.errors.push(SemanticError::new(
        ...
        match expr {
            Expr::Literal(lit) => {
                let type_hash = match &lit.kind {
                    LiteralKind::Int(_) => primitive_hashes::INT32,
    ...
    fn register_mixin_method(
        ...
        let func_hash = if is_constructor {
            ...
            crate::types::TypeHash::from_method(owner_hash, method.name.name, &param_hashes, is_const, return_is_const)
        };
            ...
            if let ClassMember::Method(method) = member {
                ...
                if method.attrs.override_ {
                    ...
                    let return_type = method
                        ...
                        .unwrap_or_else(|| DataType::simple(primitive_hashes::VOID));
...
mod tests {
    ...
    fn resolve_primitive_type() {
        ...
        if let TypeDef::Class { fields, .. } = typedef {
            ...
            assert_eq!(fields[0].data_type.type_hash, primitive_hashes::INT32);
        } else {
    ...
    fn resolve_multiple_field_types() {
        ...
        if let TypeDef::Class { fields, .. } = typedef {
            ...
            assert_eq!(fields[0].data_type.type_hash, primitive_hashes::INT32);
            assert_eq!(fields[1].data_type.type_hash, primitive_hashes::FLOAT);
            assert_eq!(fields[2].data_type.type_hash, primitive_hashes::BOOL);
            assert_eq!(fields[3].data_type.type_hash, primitive_hashes::DOUBLE);
        } else {
            panic!("Expected Class typedef");
        }
    }
    ...
    fn operator_methods_registered() {
        ...
        if let TypeDef::Class { operator_methods, .. } = typedef {
            ...
            let int_type_id = primitive_hashes::INT32;
    ...
    fn typedef_alias() {
        ...
        if let TypeDef::Class { fields, .. } = typedef {
            ...
            assert_eq!(fields[0].data_type.type_hash, primitive_hashes::FLOAT);
        } else {
            panic!("Expected Class typedef");
        }
    }
    ...
    fn funcdef_with_params() {
        ...
        if let TypeDef::Funcdef { params, return_type, .. } = typedef {
            ...
            assert_eq!(params[0].type_hash, primitive_hashes::INT32);
            assert_eq!(params[1].type_hash, primitive_hashes::FLOAT);
            assert_eq!(return_type.type_hash, primitive_hashes::INT32);
        } else {
        ...
        if let TypeDef::Class { methods, .. } = typedef {
            ...
            for method_id in methods {
                ...
                if func.name() == "getResult" {
                    ...
                    assert_eq!(func.return_type().type_hash, primitive_hashes::INT32);
                }
            }
        } else {
            panic!("Expected Class typedef");
