...
//!
//! This module implements the second pass of semantic analysis which takes the ScriptRegistry
...
use crate::semantic::types::{
    ...
    registry::ScriptParam,
    type_def::{AutoGeneratedMethod, FieldDef, FunctionTraits, MethodSignature, OperatorBehavior, TypeDef, Visibility},
};
...
use angelscript_parser::ast::expr::{Expr, InitElement};
use angelscript_parser::ast::stmt::{Block, Stmt, ForInit};
use angelscript_parser::ast::types::{ParamType, PrimitiveType, TypeBase, TypeExpr, TypeSuffix};
use angelscript_parser::ast::RefKind;
...
pub struct TypeCompilationData<'ast> {
    /// CompilationContext with complete type information (FFI + Script)
...
///
/// Walks the AST and fills in all type details in the CompilationContext.
pub struct TypeCompiler<'ast> {
    /// The compilation context we're filling in
    context: CompilationContext<'ast>,
...
impl<'ast> TypeCompiler<'ast> {
    ...
    fn new(context: CompilationContext<'ast>) -> Self {
        Self {
            context,
    ...
    fn visit_function(&mut self, func: &FunctionDecl<'ast>, object_type: Option<TypeHash>) {
        use crate::semantic::types::ScriptParam;

        let qualified_name = self.build_qualified_name(func.name.name);

        // Resolve parameter types and build ScriptParams
        // Note: void parameters can't occur from parsing (syntax doesn't allow `void param_name`)
        let mut params: Vec<ScriptParam<'ast>> = Vec::with_capacity(func.params.len());
        for p in func.params.iter() {
            if let Some(data_type) = self.resolve_param_type(&p.ty) {
                let name = p.name.map(|n| n.name.to_string()).unwrap_or_default();
        ...
        self.context.update_function_signature(&qualified_name, params, return_type, object_type, traits);
    ...
    fn visit_class(&mut self, class: &ClassDecl<'ast>) {
        ...
        for (i, inherited_ident) in class.inheritance.iter().enumerate() {
            ...
            if self.context.is_mixin(&inherited_name) {
                mixins.push(inherited_name);
                continue;
            }

            if let Some(inherited_id) = self.context.lookup_type(&inherited_name) {
                let inherited_typedef = self.context.get_type(inherited_id);

                // First item can be a base class (if it's a class type)
                if i == 0 && inherited_typedef.is_class() {
                    // Script classes cannot extend native/FFI classes
                    // Check if it's an FFI type by trying to look it up in the FFI registry
                    if self.context.ffi().get_type(inherited_id).is_some() {
                        self.errors.push(SemanticError::new(
                            SemanticErrorKind::CannotExtendNativeClass,
                            inherited_ident.span,
                            format!("script class cannot extend native class '{}' - only interfaces can be implemented", inherited_name),
                        ));
                    ...
                    if self.context.is_class_final(inherited_id) {
                        self.errors.push(SemanticError::new(
                    ...
                    if self.context.script().would_create_circular_inheritance(type_id, inherited_id) {
                        self.errors.push(SemanticError::new(
        ...
        for mixin_name in &mixins {
            if let Some(mixin) = self.context.lookup_mixin(mixin_name) {
                // Add interfaces required by the mixin
                for iface_name in &mixin.required_interfaces {
                    if let Some(iface_id) = self.context.lookup_type(iface_name)
        ...
        for member in class.members {
            if let ClassMember::Field(field) = member
                && let Some(field_type) = self.resolve_type_expr(&field.ty) {
                    ...
                    if field_type.type_hash == primitives::VOID {
                        self.errors.push(SemanticError::new(
        ...
        let inherited_field_names: FxHashSet<String> = if let Some(base_id) = base_class {
            let base_fields = self.context.get_class_fields(base_id);
        ...
        for member in class.members {
            if let ClassMember::Method(method) = member {
                ...
                // Use script registry directly - AST methods are always script functions
                let method_qualified_name = self.build_qualified_name(method.name.name);
                let all_funcs = self.context.script().lookup_functions(&method_qualified_name);
                let mut func_ids = Vec::with_capacity(all_funcs.len().min(4));
                for &id in all_funcs {
                    if self.context.script().get_function(id).object_type == Some(type_id) {
                        func_ids.push(id);
                    }
                }
        ...
        // Use script registry directly since this is a script class
        let constructors = self.context.script().find_constructors(type_id);

        // Get the type_hash for this class
        let class_type_hash = self.context.get_type(type_id).type_hash();

        for &ctor_id in &constructors {
            let func = self.context.script().get_function(ctor_id);
            ...
            if let Some(AutoGeneratedMethod::CopyConstructor) = func.traits.auto_generated {
                ...
                self.context.update_function_params(ctor_id, vec![ScriptParam {
        ...
        if let Some(op_assign_id) = self.context.script().find_operator_method(type_id, OperatorBehavior::OpAssign) {
            let func = self.context.script().get_function(op_assign_id);
            ...
            if let Some(AutoGeneratedMethod::OpAssign) = func.traits.auto_generated {
                ...
                self.context.update_function_params(op_assign_id, vec![ScriptParam {
                    name: "other".to_string(),
                    data_type: param_type,
                    default: None,
                }]);
                self.context.update_function_return_type(op_assign_id, return_type);
            }
        }
                ...
                for accessor in prop.accessors {
                    match accessor.kind {
                        angelscript_parser::ast::PropertyAccessorKind::Get => {
                            ...
                            if let Some(getter_ids) = self.context.lookup_functions(&getter_qualified).first() {
                        ...
                        angelscript_parser::ast::PropertyAccessorKind::Set => {
                            ...
                            if let Some(setter_ids) = self.context.lookup_functions(&setter_qualified).first() {
        ...
        self.context.script_mut().update_class_details(type_id, fields, method_ids, base_class, interfaces, operator_methods, properties);
    ...
    fn visit_interface(&mut self, iface: &InterfaceDecl<'ast>) {
        ...
        for member in iface.members {
            if let angelscript_parser::ast::decl::InterfaceMember::Method(method) = member
        ...
        self.context.script_mut().update_interface_details(type_id, methods);
    }
    ...
    fn visit_global_var(&mut self, var: &GlobalVarDecl<'ast>) {
        ...
        self.context.register_global_var(
        ...
        match expr {
            Expr::Literal(lit) => {
                let type_hash = match &lit.kind {
                    LiteralKind::Int(_) => primitives::INT32,
                ...
                Some(DataType::simple(type_hash))
            }
            Expr::Paren(paren) => {
                // Unwrap parentheses
    ...
    fn visit_typedef(&mut self, typedef: &TypedefDecl<'ast>) {
        ...
        // Register the alias
        self.context.register_type_alias(&alias_name, base_type.type_hash);
    }
    ...
    fn visit_funcdef(&mut self, funcdef: &FuncdefDecl<'ast>) {
        ...
        self.context.script_mut().update_funcdef_signature(type_id, params, return_type);
    }
    ...
    fn resolve_type_expr(&mut self, expr: &TypeExpr<'ast>) -> Option<DataType> {
        ...
        let type_id = if !expr.template_args.is_empty() {
            ...
            // Instantiate the template using CompilationContext
            match self.context.instantiate_template(base_type_id, arg_types) {
        ...
        // Get the type_hash from the TypeDef for this type_id
        let type_hash = self.context.get_type(type_id).type_hash();
        ...
        if let TypeDef::Class { template: Some(tmpl), .. } = self.context.get_type(type_id) {
            let template_id = *tmpl;
            let array_template = self.context.lookup_type("array");
            let dict_template = self.context.lookup_type("dictionary");
            if Some(template_id) == array_template || Some(template_id) == dict_template {
    ...
    fn resolve_base_type(
        ...
        match base {
            ...
            TypeBase::Named(ident) => {
                ...
                // Look up the type
                match self.context.lookup_type(&type_name) {
    ...
    #[inline]
    fn primitive_to_type_id(&self, prim: PrimitiveType) -> TypeHash {
        match prim {
    ...
    fn build_scoped_name(&self, scope: &angelscript_parser::ast::Scope<'ast>, name: &str) -> String {
    ...
    fn build_inherited_name(&self, ident_expr: &angelscript_parser::ast::expr::IdentExpr) -> String {
    ...
    fn parse_operator_method(
        ...
        OperatorBehavior::from_method_name(method_name, Some(return_type.type_hash))
    }
    ...
    fn register_mixin_method(
        ...
        type_id: TypeHash,
        method_ids: &mut Vec<TypeHash>,
        operator_methods: &mut FxHashMap<OperatorBehavior, Vec<TypeHash>>,
        explicit_properties: &mut FxHashMap<String, crate::semantic::types::PropertyAccessors>,
    ) {
        // Resolve parameters with types and defaults
        let params: Vec<ScriptParam<'ast>> = method
            .params
            .iter()
            .filter_map(|p| {
                let data_type = self.resolve_param_type(&p.ty)?;
                let name = p.name.map(|n| n.name.to_string()).unwrap_or_default();
                Some(ScriptParam {
                    name,
                    data_type,
        ...
        // Compute func_hash from resolved params - this IS the function identity
        let owner_hash = self.context.get_type(type_id).type_hash();
        let param_hashes: Vec<angelscript_core::TypeHash> = params.iter()
            .map(|p| self.context.get_type(p.data_type.type_hash).type_hash())
            .collect();
        let return_is_const = return_type.is_const;
        let func_hash = if is_constructor {
            angelscript_core::TypeHash::from_constructor(owner_hash, &param_hashes)
        } else {
            angelscript_core::TypeHash::from_method(owner_hash, method.name.name, &param_hashes, is_const, return_is_const)
        };
        ...
        self.context.script_mut().add_method_to_class(type_id, func_hash);
        method_ids.push(func_hash);

        // Check if this is an operator method
        if let Some(op_behavior) = self.parse_operator_method(method.name.name, &return_type) {
            operator_methods.entry(op_behavior).or_default().push(func_hash);
        }
        ...
        if method.attrs.property
            && let Some((prop_name, is_getter)) = self.parse_property_method(method.name.name) {
                ...
                if is_getter {
                    accessor.getter = Some(func_hash);
                } else {
                    accessor.setter = Some(func_hash);
                }
            }
    }
    ...
    fn validate_interface_implementation(
        ...
        let interfaces = self.context.get_all_interfaces(type_id);

        for interface_id in interfaces {
            // Get interface method signatures
            let interface_methods = match self.context.get_interface_methods(interface_id) {
                Some(methods) => methods.to_vec(), // Clone to avoid borrow issues
                None => continue,
            };

            let interface_name = self.context.get_type(interface_id).qualified_name().to_string();

            // Check each interface method is implemented
            for method_sig in &interface_methods {
                if !self.context.has_method_matching_interface(type_id, method_sig) {
                    self.errors.push(SemanticError::new(
            ...
            if let ClassMember::Method(method) = member {
                ...
                if method.attrs.override_ {
                    ...
                    let return_type = method
                        ...
                        .unwrap_or_else(|| DataType::simple(primitives::VOID));

                    // Check there's a base method to override
                    let base_method = self.context.script().find_base_method_with_signature(
                        type_id,
                        method_name,
                        &params,
                        &return_type,
                ...
                if let Some(final_method_id) = self.context.script().is_base_method_final(type_id, method_name) {
                    let base_func = self.context.script().get_function(final_method_id);
                    self.errors.push(SemanticError::new(
...
mod tests {
    ...
    fn resolve_primitive_type() {
        ...
        let player_id = data.context.lookup_type("Player").unwrap();
        let typedef = data.context.get_type(player_id);

        if let TypeDef::Class { fields, .. } = typedef {
            assert_eq!(fields.len(), 1);
            assert_eq!(fields[0].name, "health");
            assert_eq!(fields[0].data_type.type_hash, primitives::INT32);
        } else {
    ...
    fn resolve_multiple_field_types() {
        ...
        if let TypeDef::Class { fields, .. } = typedef {
            ...
            assert_eq!(fields[0].data_type.type_hash, primitives::INT32);
            assert_eq!(fields[1].data_type.type_hash, primitives::FLOAT);
            assert_eq!(fields[2].data_type.type_hash, primitives::BOOL);
            assert_eq!(fields[3].data_type.type_hash, primitives::DOUBLE);
        } else {
            panic!("Expected Class typedef");
        }
    }
    ...
    fn resolve_user_defined_type() {
        ...
        let player_id = data.context.lookup_type("Player").unwrap();
        let position_id = data.context.lookup_type("Position").unwrap();
        let typedef = data.context.get_type(player_id);

        if let TypeDef::Class { fields, .. } = typedef {
            assert_eq!(fields.len(), 1);
            assert_eq!(fields[0].data_type.type_hash, position_id);
        } else {
            panic!("Expected Class typedef");
        }
    }
    ...
    // Template tests will be added once ScriptRegistry::instantiate_template is available
    ...
    fn operator_methods_registered() {
        ...
        let data = compile(r#"
            ...
            class Vector3 {
                ...
                Target opConv() const { return Target(); }
                int opImplConv() const { return 0; }
            }
        "#, &arena);
        assert!(data.errors.is_empty(), "Errors: {:?}", data.errors);

        let vector3_id = data.context.lookup_type("Vector3").unwrap();
        let target_id = data.context.lookup_type("Target").unwrap();
        let typedef = data.context.get_type(vector3_id);

        if let TypeDef::Class { operator_methods, .. } = typedef {
            // Check opConv (explicit value conversion to Target)
            let op_conv_behavior = OperatorBehavior::OpConv(target_id);
            assert!(operator_methods.contains_key(&op_conv_behavior),
                "opConv to Target should be registered");

            // Check opImplConv (implicit value conversion to int)
            let int_type_id = primitives::INT32;
    ...
    fn operator_cast_methods_registered() {
        ...
        let container_id = data.context.lookup_type("Container").unwrap();
        let base_id = data.context.lookup_type("Base").unwrap();
        let derived_id = data.context.lookup_type("Derived").unwrap();
        let typedef = data.context.get_type(container_id);
    ...
    fn explicit_property_detection() {
        ...
        if let TypeDef::Class { properties, .. } = typedef {
            ...
            let getter_func = data.context.get_function(health_prop.getter.unwrap());
            assert!(getter_func.traits().is_const, "Getter must be const");
            assert_eq!(getter_func.name(), "get_health");

            // Verify setter is not const
            let setter_func = data.context.get_function(health_prop.setter.unwrap());
    ...
    fn typedef_creates_alias() {
        ...
        let real_id = data.context.lookup_type("real");
        let float_id = data.context.lookup_type("float");
    ...
    fn typedef_in_namespace() {
        ...
        let number_id = data.context.lookup_type("Math::number");
        let double_id = data.context.lookup_type("double");
    ...
    fn mixin_basic_method() {
        ...
        let class_id = data.context.lookup_type("MyClass").expect("MyClass not found");
        let methods = data.context.get_methods(class_id);
        let method_names: Vec<String> = methods.iter()
            .map(|&id| data.context.script().get_function(id).name.clone())
            .collect();
        assert!(method_names.iter().any(|n| n == "mixinMethod"), "Class should have mixinMethod from mixin. Methods: {:?}", method_names);
    }
    ...
    fn mixin_basic_field() {
        ...
        let class_id = data.context.lookup_type("MyClass").expect("MyClass not found");
        let fields = data.context.get_class_fields(class_id);
    ...
    fn mixin_class_method_overrides_mixin() {
        ...
        let update_count = methods.iter()
            .filter(|&&id| data.context.script().get_function(id).name == "update")
            .count();
    ...
    fn mixin_with_interface() {
        ...
        let class_id = data.context.lookup_type("Widget").expect("Widget not found");
        let typedef = data.context.get_type(class_id);
        if let crate::semantic::TypeDef::Class { interfaces, .. } = typedef {
            let iface_id = data.context.lookup_type("IDrawable").expect("IDrawable not found");
    ...
    fn multiple_mixins() {
        ...
        let method_names: Vec<String> = methods.iter()
            .map(|&id| data.context.script().get_function(id).name.clone())
            .collect();
        assert!(method_names.iter().any(|n| n == "methodA"), "Should have methodA. Methods: {:?}", method_names);
        assert!(method_names.iter().any(|n| n == "methodB"), "Should have methodB. Methods: {:?}", method_names);
    }
    ...
    fn mixin_with_base_class() {
        ...
        let class_id = data.context.lookup_type("Derived").expect("Derived not found");
        let methods = data.context.get_methods(class_id);
        let method_names: Vec<String> = methods.iter()
            .map(|&id| data.context.script().get_function(id).name.clone())
            .collect();
        assert!(method_names.iter().any(|n| n == "mixinMethod"), "Should have mixinMethod. Methods: {:?}", method_names);
    }
    ...
    fn mixin_is_not_a_type() {
        ...
        assert!(data.context.lookup_type("MyMixin").is_none(), "Mixin should not be registered as a type");
        // But it should be registered as a mixin
        assert!(data.context.is_mixin("MyMixin"), "MyMixin should be registered as a mixin");
    }
    ...
    fn typedef_alias() {
        ...
        if let TypeDef::Class { fields, .. } = typedef {
            ...
            assert_eq!(fields[0].data_type.type_hash, primitives::FLOAT);
        } else {
            panic!("Expected Class typedef");
        }
    }
    ...
    fn funcdef_with_params() {
        ...
        let callback_id = data.context.lookup_type("Callback").unwrap();
        let typedef = data.context.get_type(callback_id);

        if let TypeDef::Funcdef { params, return_type, .. } = typedef {
            assert_eq!(params.len(), 2);
            assert_eq!(params[0].type_hash, primitives::INT32);
            assert_eq!(params[1].type_hash, primitives::FLOAT);
            assert_eq!(return_type.type_hash, primitives::INT32);
        } else {
    ...
    fn interface_method_signatures() {
        ...
        let drawable_id = data.context.lookup_type("IDrawable").unwrap();
        let typedef = data.context.get_type(drawable_id);
    ...
    fn class_implementing_interface() {
        ...
        let widget_id = data.context.lookup_type("Widget").unwrap();
        let drawable_id = data.context.lookup_type("IDrawable").unwrap();
        let typedef = data.context.get_type(widget_id);
    ...
    fn global_variable_registration() {
        ...
        // Note: The exact API depends on the ScriptRegistry implementation
    ...
    fn namespace_types() {
        ...
        let player_id = data.context.lookup_type("Game::Player").unwrap();
        let typedef = data.context.get_type(player_id);
    ...
    fn nested_namespace() {
        ...
        let entity_id = data.context.lookup_type("Game::World::Entity").unwrap();
        let typedef = data.context.get_type(entity_id);
        assert!(typedef.is_class());
    }
    ...
    fn scoped_type_reference() {
        ...
        let player_id = data.context.lookup_type("Player").unwrap();
        let vector_id = data.context.lookup_type("Lib::Vector").unwrap();
        let typedef = data.context.get_type(player_id);

        if let TypeDef::Class { fields, .. } = typedef {
            assert_eq!(fields.len(), 1);
            assert_eq!(fields[0].data_type.type_hash, vector_id);
        } else {
            panic!("Expected Class typedef");
        }
    }
    ...
    // Note: Template instantiation tests for FFI types (array, dictionary) should be done
    // using compile_with_modules with the appropriate modules imported.
    // User-defined template classes (class Box<T>) require separate implementation.
    ...
    fn method_return_type_resolution() {
        ...
        let player_id = data.context.lookup_type("Player").unwrap();
        let result_id = data.context.lookup_type("Result").unwrap();
        let typedef = data.context.get_type(player_id);

        if let TypeDef::Class { methods, .. } = typedef {
            // Find getResult method and check return type
            for method_id in methods {
                let func = data.context.get_function(*method_id);
                if func.name() == "getResult" {
                    assert_eq!(func.return_type().type_hash, result_id);
                    assert!(func.return_type().is_handle);
                } else if func.name() == "getHealth" {
                    assert_eq!(func.return_type().type_hash, primitives::INT32);
                }
            }
        } else {
            panic!("Expected Class typedef");
