...
pub struct TypeCompilationData<'ast> {
    /// ScriptRegistry with complete type information
...
///
/// Walks the AST and fills in all type details in the ScriptRegistry.
pub struct TypeCompiler<'ast> {
    /// The registry we're filling in
    registry: ScriptRegistry<'ast>,
...
impl<'ast> TypeCompiler<'ast> {
    ...
    fn new(registry: ScriptRegistry<'ast>) -> Self {
        Self {
            registry,
    ...
    fn resolve_type_expr(&mut self, expr: &TypeExpr<'ast>) -> Option<DataType> {
        ...
        if let TypeDef::Class { template: Some(tmpl), .. } = self.registry.get_type(type_id) {
            let template_id = *tmpl;
            let array_template = self.registry.lookup_type("array");
            let dict_template = self.registry.lookup_type("dictionary");
...
mod tests {
    ...
    fn operator_methods_registered() {
        ...
        let target_id = data.registry.lookup_type("Target").unwrap();
    ...
    fn method_return_type_resolution() {
        ...
        let player_id = data.registry.lookup_type("Player").unwrap();
        let result_id = data.registry.lookup_type("Result").unwrap();
        let typedef = data.registry.get_type(player_id);
