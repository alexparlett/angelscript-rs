...
use crate::semantic::{INT32_TYPE, INT8_TYPE, INT16_TYPE, INT64_TYPE, UINT32_TYPE, UINT8_TYPE, UINT16_TYPE, UINT64_TYPE, FLOAT_TYPE, DOUBLE_TYPE, BOOL_TYPE, VOID_TYPE};
use crate::ast::decl::{
...
impl<'ast> TypeCompiler<'ast> {
            ...
            Expr::Literal(lit) => {
                let type_id = match &lit.kind {
                    ...
                    LiteralKind::String(_) => {
                        ...
                        self.context.lookup_type("string")?
                    }
    ...
    fn visit_typedef(&mut self, typedef: &TypedefDecl<'ast>) {
        ...
        // Register the alias
        self.context.register_type_alias(&alias_name, base_type.type_id);
    }
    ...
    fn register_mixin_method(
        ...
        let func_id = self.context.next_function_id();
...
mod tests {
    ...
    fn resolve_primitive_type() {
        ...
        if let TypeDef::Class { fields, .. } = typedef {
            ...
            assert_eq!(fields[0].data_type.type_id, INT32_TYPE);
        } else {
    ...
    fn resolve_multiple_field_types() {
        ...
        if let TypeDef::Class { fields, .. } = typedef {
            ...
            assert_eq!(fields[0].data_type.type_id, INT32_TYPE);
            assert_eq!(fields[1].data_type.type_id, FLOAT_TYPE);
            assert_eq!(fields[2].data_type.type_id, BOOL_TYPE);
            assert_eq!(fields[3].data_type.type_id, DOUBLE_TYPE);
        } else {
            panic!("Expected Class typedef");
        }
    }
    ...
    fn typedef_alias() {
        ...
        if let TypeDef::Class { fields, .. } = typedef {
            ...
            assert_eq!(fields[0].data_type.type_id, FLOAT_TYPE);
        } else {
            panic!("Expected Class typedef");
        }
    }
    ...
    fn funcdef_with_params() {
        ...
        if let TypeDef::Funcdef { params, return_type, .. } = typedef {
            ...
            assert_eq!(params[1].type_id, FLOAT_TYPE);
        ...
        if let TypeDef::Class { methods, .. } = typedef {
            ...
            for method_id in methods {
                ...
                if func.name() == "getResult" {
                    assert_eq!(func.return_type().type_id, result_id);
