...
impl<'ast> TypeCompiler<'ast> {
    ...
    fn visit_class(&mut self, class: &ClassDecl<'ast>) {
        ...
        for (i, inherited_ident) in class.inheritance.iter().enumerate() {
            ...
            if let Some(inherited_id) = self.context.lookup_type(&inherited_name) {
                ...
                if i == 0 && inherited_typedef.is_class() {
                    ...
                    if self.context.would_create_circular_inheritance(type_id, inherited_id) {
                        self.errors.push(SemanticError::new(
        ...
        for member in class.members {
            if let ClassMember::Method(method) = member {
                ...
                let all_funcs = self.context.lookup_functions(&method_qualified_name);
                let mut func_ids = Vec::with_capacity(all_funcs.len().min(4));
                for &id in all_funcs {
                    if self.context.get_script_function(id).object_type == Some(type_id) {
                        func_ids.push(id);
                    }
                }
        ...
        let constructors = self.context.find_constructors(type_id);

        for &ctor_id in &constructors {
            let func = self.context.get_script_function(ctor_id);
        ...
        if let Some(op_assign_id) = self.context.find_operator_method(type_id, OperatorBehavior::OpAssign) {
            let func = self.context.get_script_function(op_assign_id);
        ...
        self.context.update_class_details(type_id, fields, method_ids, base_class, interfaces, operator_methods, properties);
    ...
    fn visit_interface(&mut self, iface: &InterfaceDecl<'ast>) {
        ...
        self.context.update_interface_details(type_id, methods);
    }
    ...
    fn visit_funcdef(&mut self, funcdef: &FuncdefDecl<'ast>) {
        ...
        self.context.update_funcdef_signature(type_id, params, return_type);
    }
        ...
        for member in class.members {
            if let ClassMember::Method(method) = member {
                ...
                if method.attrs.override_ {
                    ...
                    let base_method = self.context.find_base_method_with_signature(
                        type_id,
                        method_name,
                        &params,
                        &return_type,
                ...
                if let Some(final_method_id) = self.context.is_base_method_final(type_id, method_name) {
                    let base_func = self.context.get_script_function(final_method_id);
                    self.errors.push(SemanticError::new(
...
mod tests {
    ...
    fn mixin_basic_method() {
        ...
        let method_names: Vec<String> = methods.iter()
            .map(|&id| data.context.get_script_function(id).name.clone())
            .collect();
    ...
    fn mixin_class_method_overrides_mixin() {
        ...
        let update_count = methods.iter()
            .filter(|&&id| data.context.get_script_function(id).name == "update")
            .count();
