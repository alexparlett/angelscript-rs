...
use crate::semantic::types::{
    ...
    type_def::{AutoGeneratedMethod, FunctionId, FunctionTraits, TypeDef, TypeId, Visibility, VOID_TYPE},
    DataType,
};
...
impl<'ast> Registrar<'ast> {
    ...
    fn visit_function(&mut self, func: &FunctionDecl<'ast>, object_type: Option<TypeId>) {
        ...
        let func_hash = if let Some(owner_id) = object_type {
            ...
            if func.is_constructor() {
                ...
                TypeHash::from_method(owner_hash, func.name.name, &param_hashes)
            }
        } else {
            // Global function
    ...
    /// Params will be filled in Pass 2a
    fn generate_copy_constructor(&mut self, class_name: &str, qualified_name: &str, type_id: TypeId) {
        let func_id = FunctionId::next();
    ...
    fn generate_op_assign(&mut self, qualified_name: &str, type_id: TypeId) {
        let func_id = FunctionId::next();

        // opAssign takes const ClassName& - hash from the qualified name
        let owner_hash = self.context.get_type(type_id).type_hash();
        let param_hash = TypeHash::from_name(&format!("const {}&", qualified_name));
        let func_hash = TypeHash::from_method(owner_hash, "opAssign", &[param_hash]);
        ...
        let func_def = FunctionDef {
            ...
            namespace: self.namespace_path.clone(),
            params: Vec::new(),  // Empty - will be filled in Pass 2a with const ClassName&
            return_type: DataType::simple(VOID_TYPE),  // Will be updated in Pass 2a to ClassName&
            object_type: Some(type_id),
            traits: FunctionTraits {
        ...
        for accessor in prop.accessors {
            match accessor.kind {
                PropertyAccessorKind::Get => {
                    ...
                    let func_hash = TypeHash::from_method(owner_hash, &getter_name, &[]);
                    ...
                    let func_def = FunctionDef {
                        ...
                        return_type: DataType::simple(VOID_TYPE),  // Will be updated in Pass 2a
                        object_type: Some(type_id),
                        traits: FunctionTraits {
                            is_constructor: false,
                            is_destructor: false,
                ...
                PropertyAccessorKind::Set => {
                    ...
                    // Setter takes one parameter of the property type
                    let param_hash = TypeHash::from_name(&format!("{}", prop.ty));
                    let func_hash = TypeHash::from_method(owner_hash, &setter_name, &[param_hash]);
