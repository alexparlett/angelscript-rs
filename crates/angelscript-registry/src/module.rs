...
impl Module {
    ...
    ///     .ty::<Player>()?
    ///     .function_meta(Player::update__meta)?;
    /// ```
    ...
    /// Register a function using an explicit FunctionMeta.
    pub fn function(mut self, meta: FunctionMeta) -> Self {
        self.functions.push(meta);
        self
    }

    /// Register a function using its macro-generated metadata function.
    ///
    /// This is the primary way to register functions. The `#[angelscript::function]`
    /// macro generates a `method__meta` companion function that returns `FunctionMeta`.
    ///
    /// # Example
    ///
    /// ```ignore
    ...
    /// fn greet(name: &str) { ... }
    ///
    /// // The macro generates greet__meta()
    /// let module = Module::new().function_meta(greet__meta)?;
    ///
    /// // For methods:
    /// let module = Module::new()
    ///     .ty::<Player>()?
    ///     .function_meta(Player::update__meta)?
    ///     .function_meta(Player::health__meta)?;
    /// ```
    pub fn function_meta(mut self, meta_fn: fn() -> FunctionMeta) -> Self {
        self.functions.push(meta_fn());
        self
    }
