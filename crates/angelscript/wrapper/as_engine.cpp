...
int asEngine_RegisterObjectMethod(asIScriptEngine *engine, const char *obj, const char *declaration, asFUNCTION_t funcPointer, asDWORD callConv) {
...
int asEngine_RegisterObjectBehaviour(asIScriptEngine *engine, const char *obj, asEBehaviours behaviour, const char *declaration, asFUNCTION_t funcPointer, asDWORD callConv) {
    if (!engine || !obj || !declaration || !funcPointer) return asINVALID_ARG;
...
void asEngine_RegisterStd(asIScriptEngine *engine) {
    if (!engine) return;
