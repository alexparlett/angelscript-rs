...
pub struct GenericParamMeta {
    ...
    /// Whether this parameter is const (immutable).
    pub is_const: bool,
}
...
pub struct ParamMeta {
    ...
    pub if_handle_then_const: bool,
    /// Reference mode for this parameter (None, In, Out, InOut).
    ///
    /// - `None` - Pass by value
    /// - `In` - `&in` reference (read-only, accepts any value)
    /// - `Out` - `&out` reference (write-only, requires mutable lvalue)
    /// - `InOut` - `&inout` reference (read-write, requires mutable lvalue)
    pub ref_mode: crate::RefModifier,
    /// Whether this parameter is const (immutable).
    ///
    /// For `&in` parameters, this is typically true (Rust `&T` is immutable).
    /// For `&out` or `&inout` parameters, this is typically false.
    pub is_const: bool,
}
    ...
    fn function_meta_basic() {
        let meta = FunctionMeta {
            ...
            params: vec![ParamMeta {
                ...
                ref_mode: crate::RefModifier::None,
                is_const: false,
            }],
            generic_params: vec![],
