...
use crate::{TypeHash, TypeKind, Operator, RefModifier};
...
pub struct ReturnMeta {
    /// Rust TypeId of the return type. None for void or variable type (`?`).
...
pub struct GenericParamMeta {
    /// Rust TypeId of the parameter type. None for variable/any type (`?`).
...
pub enum ListPatternMeta {
    ...
    /// Contains the TypeId of the repeated element type.
    Repeat(TypeId),
    /// Fixed pattern: `{T1, T2, T3}` - exact tuple of types.
    /// Contains TypeIds in order.
    Fixed(Vec<TypeId>),
    /// Repeat tuple pattern: `{{K, V}, {K, V}, ...}` - variable number of tuples.
    /// Contains TypeIds of tuple elements.
...
pub struct FunctionMeta {
    ...
    /// Return type's Rust TypeId (legacy, prefer return_meta).
...
mod tests {
    ...
    fn function_meta_generic() {
        ...
        assert!(meta.generic_params[0].param_type.is_none());
    ...
    fn return_meta_variations() {
        ...
        let value_return = ReturnMeta {
            return_type: Some(TypeId::of::<i32>()),
        ...
        let const_ref_return = ReturnMeta {
            return_type: Some(TypeId::of::<String>()),
    ...
    fn list_pattern_meta() {
        // Repeat pattern
        let repeat = ListPatternMeta::Repeat(TypeId::of::<i32>());
