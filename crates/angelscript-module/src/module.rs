...
pub struct Module<'app> {
    ...
    /// Registered native functions (FunctionDef + optional NativeFn pairs)
...
impl<'app> Module<'app> {
    ...
    /// Internal helper to build a FunctionDef from parsed signature.
    fn build_function_def(
        &self,
        sig: FunctionSignatureDecl<'_>,
        native_fn: NativeFn,
    ) -> (FunctionDef, Option<NativeFn>) {
        // Convert params
        ...
        // Build the function definition immediately
        FunctionBuilder::new(sig.name.name.to_string())
    ...
    pub fn functions(&self) -> &[(FunctionDef, Option<NativeFn>)] {
        &self.functions
    }
    ...
    fn build_funcdef_def(&self, fd: FuncdefDecl<'_>) -> FfiFuncdefDef {
        // Convert params to owned Param
        let params: Vec<Param> = fd.params.iter().map(function_param_to_ffi).collect();
    ...
    pub fn install_into(&self, builder: &mut FfiRegistryBuilder) -> Result<(), ModuleError> {
        ...
        for (func_def, native_fn) in &self.functions {
            self.install_function(builder, func_def, native_fn.as_ref());
        }
    ...
    fn install_type(
        ...
        let template_param_ids: Vec<TypeHash> = type_def
            ...
            .map(|(index, param_name)| {
                ...
                let param_hash =
                    TypeHash::from_template_instance(owner_hash, &[TypeHash(index as u64)]);
    ...
    fn install_type_constructors(
        ...
        for ctor in &type_def.constructors {
            // Clone and substitute template params
            let mut ctor_func = ctor.clone();
            Self::substitute_template_params_func(&mut ctor_func, template_param_map);

            // Add to behaviors
            let behaviors = builder.behaviors_mut(type_def.id);
            behaviors.constructors.push(ctor_func.func_hash);

            let native_fn = type_def.native_fns.get(&ctor.func_hash).map(|nf| nf.clone_arc());
    ...
    fn install_type_factories(
        ...
        for factory in &type_def.factories {
            // Clone and substitute template params
            let mut factory_func = factory.clone();
            Self::substitute_template_params_func(&mut factory_func, template_param_map);

            // Add to behaviors
            let behaviors = builder.behaviors_mut(type_def.id);
            behaviors.factories.push(factory_func.func_hash);

            let native_fn = type_def.native_fns.get(&factory.func_hash).map(|nf| nf.clone_arc());
        ...
        if let Some(list_behavior) = &type_def.list_factory {
            // Create a FunctionDef for the list factory
    ...
    fn install_type_methods(
        ...
        for method in &type_def.methods {
            ...
            Self::substitute_template_params_func(&mut method_func, template_param_map);

            method_hashes.push(method_func.func_hash);
            let native_fn = type_def.native_fns.get(&method.func_hash).map(|nf| nf.clone_arc());
    ...
    fn install_type_properties(
        ...
        for prop in &type_def.properties {
            ...
            builder.register_function(getter_func.build(), Some(prop.getter.clone_arc()));
            ...
            let setter_hash = if let Some(setter) = &prop.setter {
                let setter_func = FunctionBuilder::new(format!("set_{}", prop.name))
                    .with_owner_type(type_def.id)
                    .with_params(vec![Param::new("value", prop_type.clone())])
                    .with_return_type(DataType::simple(primitives::VOID));

                let hash = setter_func.func_hash;
                builder.register_function(setter_func.build(), Some(setter.clone_arc()));
                Some(hash)
            } else {
                None
            };
    ...
    fn install_type_operators(
        ...
        for func in &type_def.operators {
            // Look up the operator behavior from the map
            if let Some(&operator) = type_def.operator_behaviors.get(&func.func_hash) {
                let mut op_func = func.clone();
                Self::substitute_template_params_func(&mut op_func, template_param_map);

                operator_map
                    .entry(operator)
                    .or_default()
                    .push(op_func.func_hash);

                let native_fn = type_def.native_fns.get(&func.func_hash).map(|nf| nf.clone_arc());
                builder.register_function(op_func, native_fn);
            }
        }

        // Update the TypeDef with operators
        if let Some(TypeDef::Class {
            operator_methods, ..
        }) = builder.get_type_mut(type_def.id)
        {
    ...
    fn install_function(&self, builder: &mut FfiRegistryBuilder, func_def: &FunctionDef, native_fn: Option<&NativeFn>) {
        builder.register_function(func_def.clone(), native_fn.map(|nf| nf.clone_arc()));
    }
    ...
    /// Substitute template parameter hashes in a FunctionDef.
    ///
    /// This updates params and return_type, then recomputes the func_hash.
    fn substitute_template_params_func(
        func: &mut angelscript_core::FunctionDef,
        template_param_map: &rustc_hash::FxHashMap<TypeHash, TypeHash>,
    ) {
        ...
        let param_hashes: Vec<TypeHash> = func.params.iter().map(|p| p.data_type.type_hash).collect();
        let is_const = func.traits.is_const;
        let return_is_const = func.return_type.is_const;

        func.func_hash = if let Some(owner) = func.object_type {
            if func.traits.is_constructor {
                TypeHash::from_constructor(owner, &param_hashes)
            } else {
                TypeHash::from_method(owner, &func.name, &param_hashes, is_const, return_is_const)
            }
        } else {
            TypeHash::from_function(&func.qualified_name(), &param_hashes)
        };
    }
}
...
mod tests {
    ...
    fn module_register_global_property_i32() {
        use angelscript_parser::ast::PrimitiveType;

        let mut value: i32 = 42;
        let mut module = Module::root();

        module
            .register_global_property("int score", &mut value)
            .unwrap();
    ...
    fn module_register_global_property_const() {
        ...
        module
            .register_global_property("const double PI", &mut value)
            .unwrap();
    ...
    fn module_register_global_property_various_types() {
        ...
        module
            .register_global_property("int score", &mut i32_val)
            .unwrap();
        module
            .register_global_property("const double pi", &mut f64_val)
            .unwrap();
        module
            .register_global_property("bool enabled", &mut bool_val)
            .unwrap();
        module
            .register_global_property("string greeting", &mut string_val)
            .unwrap();
        ...
        assert!(matches!(
            module.global_properties()[1].ty.base,
            TypeBase::Primitive(PrimitiveType::Double)
        ));
        assert!(matches!(
            module.global_properties()[2].ty.base,
            TypeBase::Primitive(PrimitiveType::Bool)
        ));
        assert!(
            matches!(module.global_properties()[3].ty.base, TypeBase::Named(ident) if ident.name == "string")
        );
    }
    ...
    fn module_register_global_property_handle() {
        ...
        module
            .register_global_property("MyClass@ obj", &mut obj)
            .unwrap();
    ...
    fn module_register_global_property_const_handle() {
        ...
        module
            .register_global_property("const MyClass@ obj", &mut obj)
            .unwrap();
    ...
    fn register_fn_simple() {
        ...
        assert_eq!(module.functions()[0].0.name, "add");
        assert_eq!(module.functions()[0].0.params.len(), 2);
    }
    ...
    fn register_fn_no_params() {
        ...
        assert_eq!(module.functions().len(), 1);
        assert_eq!(module.functions()[0].0.name, "pi");
        assert_eq!(module.functions()[0].0.params.len(), 0);
    }
    ...
    fn register_fn_void_return() {
        ...
        assert_eq!(module.functions().len(), 1);
        assert_eq!(module.functions()[0].0.name, "greet");
    }
    ...
    fn register_fn_const_method() {
        ...
        assert_eq!(module.functions().len(), 1);
        assert!(module.functions()[0].0.traits.is_const);
    }
    ...
    fn register_fn_many_args() {
        ...
        assert_eq!(module.functions().len(), 1);
        assert_eq!(module.functions()[0].0.params.len(), 4);
    }
    ...
    fn register_fn_string_param() {
        ...
        assert_eq!(module.functions().len(), 1);
        assert_eq!(module.functions()[0].0.params.len(), 1);
    }
