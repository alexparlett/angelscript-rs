...
impl<'m, 'app> EnumBuilder<'m, 'app> {
    ...
    ///     .build()?;
    /// ```
    pub fn value(mut self, name: &str, value: i64) -> Result<Self, RegistrationError> {
        // Check for duplicate value names
        if self.values.iter().any(|(n, _)| n == name) {
            return Err(RegistrationError::DuplicateEnumValue {
    ...
    ///     .build()?;
    /// ```
    pub fn auto_value(mut self, name: &str) -> Result<Self, RegistrationError> {
        // Check for duplicate value names
        if self.values.iter().any(|(n, _)| n == name) {
            return Err(RegistrationError::DuplicateEnumValue {
                enum_name: self.name.clone(),
...
mod tests {
    ...
    fn enum_builder_duplicate_value_name() {
        ...
        match result.unwrap_err() {
            RegistrationError::DuplicateEnumValue { enum_name, value_name } => {
