...
use crate::{ListBehavior, NativeFn, TemplateInstanceInfo, TemplateValidation};
use crate::types::FfiPropertyDef;
use angelscript_core::{FunctionDef, OperatorBehavior, TypeHash, TypeKind};
...
pub struct FfiTypeDef {
    ...
    /// Operators
    pub operators: Vec<FunctionDef>,

    /// Operator behavior for each operator function (keyed by func_hash)
    pub operator_behaviors: FxHashMap<TypeHash, OperatorBehavior>,

    /// Native function implementations keyed by func_hash
    pub native_fns: FxHashMap<TypeHash, NativeFn>,
...
impl FfiTypeDef {
    ...
    pub fn new<T: 'static>(id: TypeHash, name: impl Into<String>, type_kind: TypeKind) -> Self {
        Self {
            ...
            properties: Vec::new(),
            operators: Vec::new(),
            operator_behaviors: FxHashMap::default(),
    ...
    /// Add a constructor.
    pub fn add_constructor(&mut self, constructor: FunctionDef) {
        self.constructors.push(constructor);
    }

    /// Add a factory.
    pub fn add_factory(&mut self, factory: FunctionDef) {
    ...
    /// Add an operator.
    pub fn add_operator(&mut self, operator: FunctionDef) {
